...
- Replies **accumulate** in one channel-wide store. Companion HTTP `list` (omit `installationId`) loads the first pages; the replies feed keeps them current. An open thread HTTP-lists that root's prefix into the same store. It does not open its own watch. A failed companion list retries once on a microtask after the first failure of an episode, then again on the next feed delivery for that collection, or when the tab is visible again.
