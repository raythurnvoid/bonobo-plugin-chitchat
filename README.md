...
- Frozen rows are not frozen any more. After the first messages-window delivery the page opens three `watchChanges` feeds (messages, replies, reactions) whose `updatedSince` is the newest `updatedAt` among those loaded documents — not the wall clock. An edit, a delete, a reply, or a reaction (including a removal marker) of a row outside the live window arrives on that feed and merges by revision. The cursor then advances to the newest `updatedAt` just seen. There is no timer.
...
- Replies **accumulate** in one channel-wide store. Companion HTTP `list` (omit `installationId`) loads the first pages; the replies feed keeps them current. An open thread HTTP-lists that root's prefix into the same store. It does not open its own watch.
...
- Every document read from a watch, a feed, or an HTTP page is runtime-validated (Zod) before use; invalid or foreign documents are dropped. A feed document that fails validation is re-read with `POST /api/v1/plugin-data/read` (omit `installationId`).
- Slot spend with a channel open is still 8 + N (N ≤ 8 private scopes): page rails 4 + N, one messages window, three change feeds. The feeds replace the old reactions window, replies window, and thread watch, so the page stays inside 16 slots and 24 server subscriptions.
